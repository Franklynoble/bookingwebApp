...
func (m *postgresDBRepo) Authenticate(email, testPassowrd string) (int, string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	//variables to hold the id of the authenticated user, and hasedpassword
	...
	err = bcrypt.CompareHashAndPassword([]byte(hashedPassword), []byte(testPassowrd))
...
//AllReservations returns a slice of all repository
...
//UpdatReservation updates a reservation in the database
...
//GetRestrictionsForRoomByDate returns restrictions by name  by date range
